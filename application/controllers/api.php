...
class Api extends CI_Controller {
    ...
    public function LMSInitialize()
    {
        ...
        $ms=round($date_array[0]*1000);

        $fechaInicial = date('Y-m-d H:i:s').".".$ms;
        $data = array(
    ...
    public function calificar()
    {
        ...
        } else if (true || $_POST["calificacion"] == 0)
        {
            ...
            $materiales = $this->evaluacion_x_material_model->obtenerMaterialesPorEvaluacion($_POST['idEvaluacion']);
            if($materiales)
            {
                $feedback .= "<br><br> Te recomendamos revisar el siguiente material:<br><ul>";
                foreach($materiales as $material){
                    $ubicacion= base_url() . "material/" . $material->id_curso . "/" . $material->id_modulo . "/" . $material->ubicacion;
                    $feedback .="<li><span class='video link-material' data-ubicacion='{$ubicacion}' data-id-material='$material->id_material'>{$material->nombre}</span></li>";
                }
                $feedback .="<ul>";
            }
        ...
        echo $feedback . ".";
        if ($_SESSION["rol"] != 1)
        { //Si no es un estudiante no continuar
            exit();
        }
        $data = array(
            'id_usuario_evaluacion' => $_SESSION[$_SESSION["idUsuario"] . "-" . $_POST['idEvaluacion']],
        ...
        if ($_POST["calificacion"] >= $umbral)
        {
            ...
            if (!in_array("1", $idLogrosObtenidos))
            {//Si no tiene el logro primer ejercicio, darselo
                $data = array(
                    'id_usuario' => $_SESSION["idUsuario"],
                    'id_logro' => 1,
                    'id_curso' => $idCurso
            ...
            if ($puntajeLogrado != - 1)
            {// Si es la primera vez que responde la pregunta
            ...
            $this->mejorTiempoPregunta($_SESSION["idUsuario"], $_POST['idEvaluacion'], $_POST['duracion']);// Vrrificar si es el mejor tiempo de la pregunta
        }
    }
    ...
    private function mejorTiempoPregunta($idUsuario, $idEvaluacion, $tiempo)
    {
        $evaluacion = $this->evaluacion_model->obtenerEvaluacion($idEvaluacion);
        if ($evaluacion[0]->mejor_tiempo==NULL || $evaluacion[0]->mejor_tiempo > $tiempo)
        {
            $where = array(
            ...
            $this->evaluacion_model->actualizar($data, $where);
        }
    }
}
